...
export default async function handler(req, res) {
  ...
  try {
    ...
    result.blobPrivate = String(stored.url || '').includes('private') || Boolean(stored.pathname);
    ...
    const speech = await fetch('https://api.openai.com/v1/audio/speech', {
      ...
      body: JSON.stringify({
        ...
        voice: 'alloy',
        input: 'Hello. This is a short Ryan Members transcription test.',
        response_format: 'mp3'
      }),
    ...
    return res.status(200).json({ ok: Object.entries(result).filter(([k]) => ['blobWrite','blobRead','tts','diarization','structuredAnalysis'].includes(k)).every(([,v]) => v === true), ...result });
  } catch (error) {
    return res.status(500).json({ ok: false, ...result, error: error.message || 'Smoke test failed' });
  } finally {
    if (blobUrl) {
      try { await del(blobUrl); result.blobDelete = true; } catch {}
    }
  }
}
