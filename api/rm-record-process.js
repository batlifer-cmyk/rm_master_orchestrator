...
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    title: { type: 'string' },
    speaker_map: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { speaker: { type: 'string' }, label: { type: 'string' } }, required: ['speaker', 'label'] } },
    summary: { type: 'string' },
    ...
    consultation: {
      ...
      properties: {
        ...
        concerns: { type: 'array', items: { type: 'string' } }, price_reaction: { type: 'string' }, registration_signal: { type: 'string' },
      },
    ...
    lesson: {
      ...
      properties: {
        ...
        corrections: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { original: { type: 'string' }, corrected: { type: 'string' }, reason: { type: 'string' } }, required: ['original', 'corrected', 'reason'] } },
...
async function transcribe(audio, filename) {
  ...
  const response = await fetch('https://api.openai.com/v1/audio/transcriptions', {
    method: 'POST', headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: form, signal: AbortSignal.timeout(280_000),
  });
...
async function analyze({ kind, subjectName, staffName, note, transcript }) {
  const segments = (transcript.segments || []).map(s => `[${s.speaker}] ${s.text}`).join('\n');
  const instructions = `You analyze Ryan Members 1:1 English academy conversations.\n- Never rewrite or translate the raw transcript.\n- Korean/English code-switching is intentional data. Preserve quoted student utterances exactly.\n- Infer speaker identities only when supported; otherwise use 화자 A, 화자 B, etc.\n- For consultations, extract only stated facts; unknown values must be empty.\n- For lessons, identify errors only when reasonably clear; never invent them.\n- registration_signal must be 높음, 중간, 낮음, 판단불가, or empty.\n- Return structured JSON only.`;
  const input = `기록 유형: ${kindLabel(kind)}\n대상자/학생: ${subjectName || '미입력'}\n상담자/강사: ${staffName || '미입력'}\n메모: ${note || '없음'}\n\n화자분리 원문:\n${segments}`;
  const response = await fetch('https://api.openai.com/v1/responses', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.OPENAI_API_KEY}` },
    body: JSON.stringify({ model: ANALYSIS_MODEL, instructions, input, text: { format: { type: 'json_schema', name: 'rm_record_analysis', strict: true, schema: ANALYSIS_SCHEMA } } }),
...
async function cleanup(chunks) {
  const urls = (chunks || []).map(x => x.url).filter(Boolean);
...
export default async function handler(req, res) {
  ...
  if (!blobConfigured()) return res.status(503).json({ error: 'Private Blob 스토리지가 아직 연결되지 않았습니다.' });

  const body = parseJsonBody(req);
  const chunks = Array.isArray(body.chunks) ? body.chunks : [];
  const kind = String(body.kind || '');
  if (!['phone', 'in_person', 'lesson'].includes(kind) || !body.uploadId || !chunks.length || body.consentConfirmed !== true) return res.status(400).json({ error: '필수 정보가 누락되었습니다.' });
  if (Number(body.size || 0) > MAX_AUDIO_BYTES) { await cleanup(chunks); return res.status(413).json({ error: '음성파일은 최대 24MB까지 전사할 수 있습니다.' }); }
  ...
  try {
    ...
    const analysis = await analyze({ kind, subjectName: String(body.subjectName || '').trim(), staffName: String(body.staffName || '').trim(), note: String(body.note || '').trim(), transcript });
    const id = crypto.randomUUID();
    const createdAt = new Date().toISOString();
    const finalAudio = await put(`rm-record/audio/${createdAt.slice(0, 10)}/${id}${safeExtension(originalName, contentType)}`, audio, { access: 'private', addRandomSuffix: false, contentType });
    const record = {
      id, createdAt, kind,
      subjectName: String(body.subjectName || '').trim(), staffName: String(body.staffName || '').trim(), note: String(body.note || '').trim(), consentConfirmed: true,
      audio: { url: finalAudio.url, pathname: finalAudio.pathname, originalName, contentType, size: audio.size }, transcript, analysis,
    };
    await put(`rm-record/records/${createdAt.slice(0, 10)}/${id}.json`, JSON.stringify(record), { access: 'private', addRandomSuffix: false, contentType: 'application/json; charset=utf-8' });
