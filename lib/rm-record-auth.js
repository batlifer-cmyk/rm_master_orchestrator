...
function serializeSessionCookie(token, maxAge) {
  // All RM Record API routes are siblings such as /api/rm-record-login and
  // /api/rm-record-google-config-set. A Path of /api/rm-record does not match
  // those hyphenated sibling routes, so scope the HttpOnly session to the site.
