...
export function isRmRecordAuthorized(req) {
  ...
  const cookieOk = verifyRmRecordAccessKey(cookieKey);

  // Legacy header support for existing clients during migration.
  const value = req.headers['x-rm-access-key'];
  const input = Array.isArray(value) ? value[0] : value;

  // The new entry page uses a harmless session marker because the real key stays HttpOnly.
  if (input === COOKIE_SESSION_MARKER) return cookieOk;
  // If a caller explicitly supplies any other header, validate that header itself.
